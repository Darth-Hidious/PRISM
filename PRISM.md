...
- **Tool chaining**: the agent chains tools across turns (e.g. search →
  evaluate → record) without being told to
...
**Materials Discovery**
- physics descriptors for a composition: delta H_mix, VEC, mixing entropy,
  density, melting point — no ML model, no torch, no GPU.
- pretrained GNN property prediction (M3GNet / MEGNet formation energy and
  band gap) via `matgl`.
- `mace_*` — MACE foundation interatomic potentials for relaxation and
  energetics; CALPHAD / pyiron simulation bridges.
...
2. **Active learning oracle** — the discovery loop works but needs a real
