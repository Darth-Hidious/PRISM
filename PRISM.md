...
- `gflownet/` — alloy-mogfn ported (MCMC sampler, physics surrogate)
...
2. **Active learning oracle** — `gfn_discover` works but needs a real
...
4. **LLM-GFlowNet** — use the LLM as a GFlowNet forward policy. The LLM
   brings chemical knowledge; GFlowNet brings diversity. Training via
   trajectory-balance loss updates the LLM to sample ∝ reward.
