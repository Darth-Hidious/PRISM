    ...
    def _atoms_from_input(self, ip: dict[str, Any], seed: int):
        ...
        if "structure" in ip:
            ...
            if s.get("composition"):
                ...
                return self._build_supercell(comp, phase, seed), comp, phase
            if s.get("cache_ref"):
                return self._atoms_from_cache_ref(s["cache_ref"])
            raise ValueError(
                "structure has neither an inline composition+phase nor a "
                "cache_ref — import one first with structure_import."
            )
        raise ValueError("input has no composition / matrix_composition / structure")

    def _atoms_from_cache_ref(self, cache_ref: str):
        """Hydrate an ASE Atoms from a cache:// CIF written by an earlier
        tool (structure_import or a previous primitive's result)."""
        import io as _io
        ...
        cache = get_mace_bridge().cache
        key, _kind = parse_cache_uri(cache_ref)
        cif = cache.read_structure_cif(key)
        if cif is None:
            raise ValueError(
                f"no cached structure for {cache_ref!r} — "
                "use structure_import or check the URI with mace_list_jobs"
            )
        atoms = ase_read(_io.StringIO(cif), format="cif")
        comp = {el: int(n) for el, n in Counter(atoms.get_chemical_symbols()).items()}
        meta = cache.read_meta(key) or {}
        return atoms, comp, meta.get("phase")

    # ------------------------------------------------------------------
    def _cif_text(self, atoms) -> str:
        from ase.io import write as ase_write

        # ase's CIF writer requires a binary stream (StringIO raises
        # "string argument expected, got 'bytes'" on ase >= 3.28).
        buf = io.BytesIO()
