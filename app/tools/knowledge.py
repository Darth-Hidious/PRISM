...
class _DirectClient:
    ...
    def _get(self, path: str, params: dict | None = None) -> dict:
        ...
        return platform().get(path, params=params or {}, timeout=15)
...
class _DirectKnowledge:
    ...
    def graph_search(self, term: str, limit: int = 10):
        return self._c._get("/knowledge/graph/search", {"q": term, "limit": str(limit)})

    def graph_entity(self, name: str, limit: int = 10):
        # NOTE the /graph segment — /knowledge/entity/{name} does not exist
        # server-side and 404s (caught live by the stress test).
        return self._c._get(f"/knowledge/graph/entity/{name}", {"limit": str(limit)})
...
def _get_client():
    ...
    if platform().authenticated:
        return _DirectClient(platform().api_url, "")

    logger.warning("No MARC27 auth — run `prism login` or set MARC27_API_KEY")
    return None
...
def _act_search(client, **kw) -> dict:
    ...
    # Default capped at 10: result items feed straight into the LLM context;
    # 20 hits measurably bloated the local model's KV cache (stress test).
    results = client.knowledge.graph_search(term, limit=kw.get("limit", 10))
    return {
        "results": results,
