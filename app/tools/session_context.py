...
def _record_evaluation(formula: str, result: dict, tool: str) -> None:
    """Record a composition evaluation (e.g. alpha_predict)."""
...
def _record_discovery(result: dict) -> None:
    """Record a discovery campaign result."""
    session = _load_session()
    ...
    for p in pareto:
        ...
        if formula:
            _record_evaluation(formula, p, "discovery")
...
def _session_context(**kwargs) -> dict:
    ...
    if action == "record":
        ...
        if tool in ("alpha_predict",) and formula:
            _record_evaluation(formula, result, tool)
        elif tool in ("discover_materials",):
...
_SESSION_CONTEXT_DESCRIPTION = (
    ...
    "its reasoning.\n\n"
    "Call this tool:\n"
    "  - After alpha_predict results: action='record'\n"
