...
///
/// MIRDYNE, not MARC27. An IRI is class IDENTITY, not branding: a class
/// minted under the old namespace names a different thing from the same
/// class minted under this one, so this is a migration and not a rename.
///
/// It is a cheap one, measured before the change: the live store held ONE
/// row with a marc27 class IRI out of 2,065 in `emmo_entity`, and none at all
/// in `ontology_term_binding`. Nearly every binding points at
/// `w3id.org/emmo#`; only induced or imported artifacts mint ours. Artifacts
/// still carrying the old namespace are regenerated by re-import or
...
pub fn ontology_iri(domain: &str) -> String {
    format!("https://prism.mirdyne.com/ontology/{domain}")
}
