...
pub async fn extract_facts_from_chunk(
    ...
    // Numeric grounding reads the unwrapped CHUNK (the text the model
    // actually saw, with soft line wraps healed); subject presence and the
    // review spans read the document. See `retain_grounded_with`.
