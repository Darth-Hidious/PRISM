...
pub async fn sync_tools(marketplace: &MarketplaceClient<'_>) -> Result<SyncReport> {
    ...
    debug!(count = remote_tools.len(), "marketplace tools fetched for sync");
    ...
    for tool in remote_tools {
            ...
            .and_then(|r| r.error_for_status())
        {
            ...
            Err(e) => {
                report
                    .failed
                    .push((slug.clone(), format!("download: {e}")));
                continue;
            }
        };
        ...
        if content.trim_start().starts_with('<') {
            report
                .failed
                .push((slug.clone(), "downloaded content looks like HTML, not Python".into()));
            continue;
        }
...
mod tests {
    ...
    fn safe_slug_rejects_traversal() {
        ...
        assert_eq!(safe_slug("predict.density.mace"), Some("predict.density.mace"));
    }
