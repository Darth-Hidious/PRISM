...
async fn run_local_text_ingest_file(
    ...
    // dropped_units + dropped_malformed + deduped + store_failed +
    // facts_written, asserted by test. `store_failed` covers facts stranded
    ...
    for (index, (start, end)) in windows.iter().enumerate() {
        ...
        let extraction = match prism_ingest::text_extract::extract_facts_from_chunk(
            &llm,
            title,
            &text[*start..*end],
    ...
    eprintln!(
        "  extraction funnel: {facts_proposed} proposed → {} written \
         ...
         {dropped_malformed} malformed, {deduped} duplicate, {store_failed} store-failed)",
        written_facts.len(),
    );
