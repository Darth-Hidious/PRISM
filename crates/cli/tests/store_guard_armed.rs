...
//! test; `cargo test -p prism-cli` builds an agent that resolves the default
...
fn the_live_store_guard_is_armed_for_this_crates_tests() {
    assert!(
        ...
        "prism-cli tests are building prism-agent WITHOUT `test-guard`: tests here \
         can open the user's live ~/.prism/provenance.db. Restore `features = \
         [\"test-guard\"]` on the prism-agent dev-dependency in crates/cli/Cargo.toml."
    );
}
