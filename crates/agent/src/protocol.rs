...
async fn handle_models_slash_command(
    ...
    match action {
        // `/models list` → structured `ui.model.list` for the TUI picker.
        "list" => {
            ...
            let norm: Vec<Value> = models
                .iter()
                .map(|m| {
                    let id = value_string(m, &["model_id", "id"]).unwrap_or("?");
                    let label = value_string(m, &["display_name", "name"]).unwrap_or(id);
                    let provider = value_string(m, &["provider"]).unwrap_or("");
                    serde_json::json!({
                        "id": id,
                        "label": label,
                        "provider": provider,
...
/// `/gpus` — the live GPU procurement catalog for the TUI picker.
///
/// Shells to `prism gpus` (which prints one JSON document on stdout: the
/// raw catalog array, or `{"error": "..."}` on failure — always exit 0)
/// and emits a structured `ui.gpu.list` notification with normalized
/// entries the picker renders directly.
async fn handle_gpus_slash_command(
    args: &[String],
    slash_ctx: &SlashCommandContext,
) -> Result<bool> {
    if args.first().map(String::as_str) != Some("gpus") {
        return Ok(false);
    }

    let value = run_cli_backed_slash_command_json(&args[..1], slash_ctx).await?;
    let error = value_string(&value, &["error"]).map(str::to_string);
    let gpus = value_array(&value, &["gpus", "items", "data"])
        .cloned()
        .unwrap_or_default();
    let norm: Vec<Value> = gpus
        .iter()
        .map(|g| {
            serde_json::json!({
                "gpu_type": value_string(g, &["gpu_type", "name"]).unwrap_or("?"),
                "vram_gb": g.get("vram_gb").and_then(|v| v.as_u64()).unwrap_or(0),
                "region": value_string(g, &["region"]).unwrap_or(""),
                "provider": value_string(g, &["provider"]).unwrap_or(""),
                "price_per_hour_usd": g
                    .get("price_per_hour_usd")
                    .and_then(|v| v.as_f64())
                    .unwrap_or(0.0),
                "available": g.get("available").and_then(|v| v.as_bool()).unwrap_or(true),
            })
        })
        .collect();
    emit_notification(
        "ui.gpu.list",
        serde_json::json!({ "gpus": norm, "error": error }),
    );
    emit_notification("ui.turn.complete", serde_json::json!({}));
    Ok(true)
}
...
/// Human-readable verb for a `ui.tool.start` notification.
///
/// Frontends display this verbatim, so it must read as a sentence
/// fragment ("Searching the web — \"NiTi damping\""), not a codename
/// ("Running web").  Detail (path / query / URL / command) is folded in
/// from the `preview` built at the call site when it is available and
/// short.  Unknown tools fall back to "Running <tool_name>".
fn humanize_tool_verb(tool_name: &str, preview: Option<&str>) -> String {
    /// Clip a detail fragment so the transcript line stays short.
    ...
    let detail = |prefix: &str| preview.and_then(|p| p.strip_prefix(prefix)).and_then(clip);

    match tool_name {
        // Previews are "search \"<query>\"" / "read <url>" (see tool_preview).
        "web" | "web_search" | "web_read" | "web_fetch" => {
            if let Some(url) = detail("read ") {
                format!("Reading a web page — {url}")
            } else if let Some(query) = detail("search ") {
                format!("Searching the web — {query}")
            } else if matches!(tool_name, "web_read" | "web_fetch") {
                "Reading a web page".to_string()
            } else {
                "Searching the web".to_string()
            }
        }
        // Previews are "<action> <path>" for the unified `file` tool and
        // the split read_file / write_file / edit_file tools alike.
        "file" | "read_file" | "write_file" | "edit_file" => detail("read ")
            .map(|p| format!("Reading {p}"))
            .or_else(|| detail("write ").map(|p| format!("Writing {p}")))
            .or_else(|| detail("edit ").map(|p| format!("Editing {p}")))
            .unwrap_or_else(|| match tool_name {
                ...
                _ => "Working with a file".to_string(),
            }),
        "execute_bash" | "bash" => match detail("$ ") {
            Some(cmd) => format!("Running a command — $ {cmd}"),
...
async fn handle_command(
    ...
    match trimmed {
        "/tools" => {
            // Also push a structured catalog so the TUI sidebar can list the
            // actual tools (not just run-activity).
            let catalog: Vec<Value> = tools
                ...
                .collect();
            emit_notification("ui.tools.catalog", serde_json::json!({ "tools": catalog }));
        ...
        _ if trimmed.starts_with("/model ") => {
            ...
            if new_model.is_empty() {
                ...
                // Keys saved via the TUI API-key window mid-session become
                // visible on switch — no backend restart needed.
    ...
    for line in reader.lines() {
        ...
        match method {
            "init" => {
                ...
                // Seed the provenance ledger context with the REAL session
                // id + model (pre-fix: literal "session", NULL model).
                crate::hooks::set_provenance_ctx(
                    runtime.session_store.current_id().unwrap_or(""),
                ...
                // Push the live tool catalog so the TUI sidebar Tools tab is
                // populated from startup (no need to run `/tools`).
                let tool_catalog_json: Vec<Value> = tools
                    .iter()
                    .map(|t| {
                        serde_json::json!({
                            "name": t.name,
                ...
                emit_notification(
                    "ui.tools.catalog",
                    serde_json::json!({ "tools": tool_catalog_json }),
                );
                emit_status_snapshot(
                    config.auto_approve,
                    &runtime.transcript,
...
mod tests {
    ...
    fn humanize_tool_verb_maps_known_tools() {
        assert_eq!(humanize_tool_verb("web", None), "Searching the web");
        assert_eq!(
            humanize_tool_verb("web", Some("search \"NiTi damping\"")),
            "Searching the web — \"NiTi damping\""
        );
        assert_eq!(
            humanize_tool_verb("web", Some("read https://x.org/p")),
            "Reading a web page — https://x.org/p"
        );
        assert_eq!(humanize_tool_verb("web_read", None), "Reading a web page");
        assert_eq!(
            humanize_tool_verb("execute_bash", Some("$ ls -la")),
            "Running a command — $ ls -la"
        );
        assert_eq!(
            humanize_tool_verb("execute_bash", None),
            "Running a command"
        );
        assert_eq!(
    ...
    #[test]
    fn humanize_tool_verb_is_action_aware_for_files() {
        assert_eq!(
            humanize_tool_verb("file", Some("read src/main.rs")),
            "Reading src/main.rs"
        );
        assert_eq!(
            humanize_tool_verb("file", Some("edit src/main.rs")),
            "Editing src/main.rs"
        );
        assert_eq!(
            humanize_tool_verb("read_file", Some("read a.txt")),
            "Reading a.txt"
        );
        assert_eq!(humanize_tool_verb("write_file", None), "Writing a file");
        assert_eq!(humanize_tool_verb("file", None), "Working with a file");
    }

    #[test]
    fn humanize_tool_verb_falls_back_and_clips() {
        assert_eq!(
            humanize_tool_verb("sample_material", None),
            "Running sample_material"
        );
        let long = format!("read {}", "x".repeat(100));
        let verb = humanize_tool_verb("file", Some(&long));
        assert!(verb.starts_with("Reading "));
        assert!(verb.ends_with('…'));
        assert!(verb.chars().count() < 64, "long details must be clipped");
    }
