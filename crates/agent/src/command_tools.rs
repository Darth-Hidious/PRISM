...
pub(crate) fn strip_platform_credentials(cmd: &mut TokioCommand) {
    ...
    // Every platform setting under BOTH names: the canonical PRISM surface and
    // every frozen MARC27 alias. Listing the spellings by hand meant the
    // neutral names this codebase now tells operators to prefer
    // (`PRISM_API_KEY`, `PRISM_TOKEN`, `PRISM_PLATFORM_URL`) were NOT stripped
    // — so following the documented advice silently removed the credential
    // boundary, and a live platform key reached an untrusted LocalOnly child.
    // Derived from `PlatformVar::ALL` so a variable added there can never again
    // be missed here, in either spelling.
