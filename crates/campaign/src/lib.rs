...
//! 2. Evaluates each candidate (via tools: gfn_evaluate, predict, DFT)
...
pub struct Candidate {
    ...
    /// Physics descriptors from gfn_evaluate or similar.
    #[serde(default)]
...
impl Campaign {
    ...
    ///
    /// In the current implementation, this calls the `gfn_evaluate` tool
    ...
    async fn evaluate_candidate(&self, composition: &str, iteration: usize) -> Result<Candidate> {
        ...
        let resp = client
            ...
            .send()
            .await
            .context("failed to call gfn_evaluate")?;
