...
// PRISM uses universal Unicode glyphs so the prompt renders correctly in any
// terminal, including those without a NerdFont installed. (Forge upstream
// used private-use NerdFont glyphs which show up as � or boxes for users
// who haven't patched their font.)
const DIR_SYMBOL: &str = "";
const BRANCH_SYMBOL: &str = "@";
const SUCCESS_SYMBOL: &str = "\u{276F}"; // ❯  heavy right-pointing chevron

const AGENT_SYMBOL: &str = "\u{25C6}"; // ◆
const MODEL_SYMBOL: &str = "\u{25CF}"; // ●
...
impl Prompt for ForgePrompt {
    ...
    fn render_prompt_right(&self) -> Cow<'_, str> {
        ...
        // Agent name with universal Unicode prefix.
        // PRISM displays the default `forge` agent as "PRISM" so the chat
        // surface reads as PRISM-branded; switching to other agents
        // (`:muse`, custom user agents) still shows the real agent name.
        let agent_label = match self.agent_id.as_str() {
            "forge" => "PRISM".to_string(),
            other => other.to_case(Case::UpperSnake),
        };
        let agent_str = format!("{AGENT_SYMBOL} {agent_label}");
        write!(
            result,
            " {}",
