...
pub fn display(cli_mode: bool) -> io::Result<()> {
    ...
    // Build tips based on mode. Both `/` and `:` prefixes are accepted by
    // the command parser; `/` is advertised since it matches the convention
    // most users (Claude Code, OpenCode, Discord-style) expect.
    ...
    // Suppressed when embedded in a host CLI (e.g. prism) via FORGE_HIDE_ZSH_TIP=1.
    let hide_zsh_tip = std::env::var("FORGE_HIDE_ZSH_TIP")
        .ok()
        .map(|v| !v.is_empty() && v != "0")
        .unwrap_or(false);
    if !cli_mode && !hide_zsh_tip {
