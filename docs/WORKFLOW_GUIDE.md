...
description: Explore diverse materials using generative sampling.
...
arguments:
  ...
  - name: model
    type: string
    default: generative-composition-v1
    help: Generative model slug from marketplace
  - name: gpu
    type: string
    default: RTX-4090
...
steps:
  ...
  - id: kg_report
    action: message
    text: "Found existing materials in {{ space }} space. Launching a generative sampler to explore beyond known candidates."

  # ──────────────────────────────────────────────────────
  # Step 4: Submit generative sampling compute job
  # ──────────────────────────────────────────────────────
  - id: submit_job
    action: http
    method: POST
    ...
    body:
      image: "marc27/generative:latest"
      name: "explore-{{ space }}"
      gpu_type: "{{ gpu }}"
      inputs:
        space: "{{ space }}"
...
```
explore  dry_run
Explore diverse materials using generative sampling.
...
## Putting It Together: Generative Exploration

The complete flow for adding a new generative exploration capability:

1. **Publish the model** to marketplace:
   ```bash
   prism publish ./generative-checkpoint --to marc27 --repo marc27/generative-composition-v1
   ```
...
steps:
  ...
  # 3. Run generative exploration (retries for flaky GPU)
  - id: explore
    action: http
    method: POST
