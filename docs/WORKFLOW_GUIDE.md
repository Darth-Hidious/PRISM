...
arguments:
  ...
  - name: model
    type: string
    default: sampler-composition-v1
    help: sampler model slug from marketplace
  - name: gpu
    type: string
    default: RTX-4090
...
steps:
  ...
  - id: kg_report
    action: message
    text: "Found existing materials in {{ space }} space. Launching the sampler to explore beyond known candidates."

  # ──────────────────────────────────────────────────────
  # Step 4: Submit sampler compute job
  # ──────────────────────────────────────────────────────
  - id: submit_job
    action: http
    method: POST
...
1. **Publish the model** to marketplace:
   ```bash
   prism publish ./sampler-checkpoint --to marc27 --repo marc27/sampler-composition-v1
   ```
