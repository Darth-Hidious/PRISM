...
>
> **Process rule (lesson from Bug #27).** When you ship code that talks to another repo's HTTP surface (the marc27-core platform, in PRISM's case), you MUST verify the other side actually exposes the endpoint you're calling. Use `mcp__codebase-memory-mcp__search_code` against the marc27-core index, search for `.route(`, dump the router. PR #38 (F1 chunk 2) and PR #43 (F1c3) both shipped with green CI, both call `/federation/...` endpoints, and the marc27-core router has **no `/federation` route group**. Unit tests passed because they mock the HTTP transport. Mock tests do NOT catch missing real endpoints — they only catch "I called the function I thought I called." The check that actually matters: `search_graph(project="...marc27-core", name_pattern="federation")` returning zero results is the test failing.
...
| 27 | **CRITICAL — platform integration gap.** F1c3's `PlatformPubkeyFetcher` (PR #43) calls `GET /federation/platform-pubkey` on `api.marc27.com`. That endpoint **does not exist on marc27-core**. The router in `crates/api/src/router.rs` exposes `/auth, /users, /orgs, /projects, /nodes, /policy, /workflows, /jobs, /compute, /knowledge, /marketplace, /admin, /admin/audit, /api-keys, /callbacks, /usage, /support, /llm-keys, /llm, /mcp-services, /graphql, /agent, /discourse, /email, /billing, /collab, /invites` — **no `/federation` group**. Furthermore, marc27-core uses **JWT/HMAC** for auth and **X25519** (key exchange) for E2EE — there is no Ed25519 platform root signing key anywhere. The whole F1 trust model (platform-signed `PeerIdentity`, distributed via published pubkey) is built on infrastructure that doesn't exist on the platform side. | NOT FIXED. Two paths: (A) add `/federation` route group + Ed25519 platform keypair to marc27-core (cross-repo PR, 1-2 days, preserves the "no platform round-trip per request" property); (B) refactor F1 to use the existing JWT-based identity (PRISM-only PR, but loses the "verifier doesn't call platform per-request" property — every cross-org request would hit `/auth/introspect`). **Recommend A.** | Verified by `mcp__codebase-memory-mcp__search_code` against marc27-core: `\.route\(\|federation\|/api/v1` in router.rs returns the full route list, `/federation` is absent. `Ed25519\|signing_key\|VerifyingKey\|platform_pubkey\|root_pubkey` returns 0 results in the entire codebase. The crypto module uses X25519 only. |
