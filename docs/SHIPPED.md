...
| 19 | mDNS peer discovery between two local prism processes returns `peer_count: 0` on both | **NOT FIXED.** Root cause identified 2026-05-09: `mdns-sd` 0.11.5 vs macOS `mDNSResponder` (Bonjour) port-5353 conflict. `dns-sd -B _prism._tcp local` (macOS native) sees zero instances even while a prism node is alive and its own logs say "mDNS: announced as prism-...". Two processes on the same Mac never see each other. Fixes ranked by cost: (A) switch to Bonjour-native bind on macOS — 1-2 weeks; (B) add static `~/.prism/peers.toml` config to bypass mDNS on macOS dev — 1 day; (C) test on Linux (Docker / head server) where mdns-sd works fine — 1-2h. **Recommend (C) first**, then (B) in parallel, queue (A). | Reproduced live: `dns-sd -B _prism._tcp local` returns 0 results for 10s while node is announcing |
