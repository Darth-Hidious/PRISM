...
NEXT: live budgeted research run (--budget-min 8) must end with an answer — verify; TUI research form gets a budget field + `[research] budget_min` config; extend `needs_human` to the patent backend (Lens token / no backend); model window lists fallbacks; bash.py `.git`/`.prism` refusal — owner's call; CONTEXT.md ≤20 lines at session end.
