...
NEXT: approve run 11 file tool; judge §12; rebuild+send .docx; finish TUI top-10 workflow (on change 2) → full gate+build+install; offer Obscura binary install + papers_fulltext wiring at that build; PRISM pre-flight checklist; owed: N1-cell QE anchor, icet CE, elastic-concurrency + 54ms-mpirun bugs, CALPHAD TDB; CONTEXT.md ≤20 lines at end.
