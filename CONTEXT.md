...
NEXT: judge §11 (RHEA) when run 9 writes it (clock 01:08); rebuild+send .docx; when the top-10 workflow finishes: review its commits (git log), run the full tui gate, build+install, respawn the TUI on the new binary (only after run 9 ends); tool descriptions: supercells by cache_ref; 54 ms mpirun death unexplained; CALPHAD TDB; CONTEXT.md ≤20 lines at session end.
