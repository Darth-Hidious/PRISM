...
NEXT: fix the large-result offload note (execute_python is a fresh process per call — `doc_N` never exists there; name notebook_exec or drop the promise); judge run 2's §9; rebuild the .docx; CONTEXT.md ≤20 lines at session end.
