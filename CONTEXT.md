...
NEXT: qe_run should accept a structure dict/formula (tools.py); find why the wrapper's mpirun died in 54 ms in run 2 (stderr now captured); Tier 2 CALPHAD TDB provisioning (OQ6); MD roadmap = post-training + known-answer gates (context store); CONTEXT.md ≤20 lines at session end.
