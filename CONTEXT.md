...
NEXT: deadline-fix commit still RED on 2 renamed-assertion tests (fix wording, gate, commit); finish novelty for O2-TF-Fe0/H5/H6/N1/N2 + verify the 10 when credits return; patent DB (needs backend); DFT campaign → cluster or drop; CALPHAD TDB; TUI top-10 workflow (change 5) → gate+build+install; PRISM pre-flight; CONTEXT.md ≤20 lines at end.
