...
NEXT: live run 3 (--budget-min 6, outer timeout 20 min) must end with an answer — verify with the stamps; if the parent synthesis is the slow part, stream partial text to stderr in --json mode; TUI research form budget field; extend `needs_human` to the patent backend; model window lists fallbacks; bash.py `.git`/`.prism` refusal — owner's call; CONTEXT.md ≤20 lines at session end.
