...
NEXT: judge run 10's §11 patch; rebuild+send .docx; when the top-10 workflow finishes: review commits, full tui gate, build+install, respawn TUIs; PRISM pre-flight beta checklist (owner: 'almost ready for its first pre-flight'); tool descriptions: supercells by cache_ref; elastic concurrency NameError; 54 ms mpirun death; CALPHAD TDB; CONTEXT.md ≤20 lines at session end.
