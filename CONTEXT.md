...
NEXT: land the deadline fix (mutant + gate + commit); TUI top-10 workflow (at change 5) → full gate + build + install + respawn; N1-cell QE anchor, icet CE, elastic/MD torch.fx concurrency bug (LOAD_LOCK does not cover it — 8 legs failed tonight), 54 ms mpirun death, CALPHAD TDB; Obscura binary install (explicit OK) + papers_fulltext wiring; PRISM pre-flight checklist; CONTEXT.md ≤20 lines at end.
