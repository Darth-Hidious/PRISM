...
NEXT: judge §10 when run 4 writes it (or restart with a shorter prompt if the provider stays at <1 tok/s); fold the QE rerun's number into the review honestly (tool rerun after fix, cutoff source); rebuild+send .docx; 54 ms mpirun death in the TUI's tool server still unexplained (stderr now captured); Tier 2 CALPHAD TDB (OQ6); CONTEXT.md ≤20 lines at session end.
