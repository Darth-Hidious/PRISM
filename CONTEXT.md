...
NEXT: source table "20 of 43" + FOUND header; clickable DOIs/links open the browser (owner reversed the no-browser rule) — staged in scratchpad links_totals.py; hover a paper → abstract; SX500/FFSC run in tmux `prism` (started 19:42:32 CEST) — time it, judge the document; rebuild+install AFTER it ends; CONTEXT.md ≤20 lines at session end.
