...
NEXT: SX500/FFSC-preburner research run in tmux `prism` (started 19:42:32 CEST) — time it, judge the document; stream partial text to stderr in --json mode; TUI research form budget field; `needs_human` for the patent backend; model window lists fallbacks; bash.py `.git`/`.prism` refusal — owner's call; CONTEXT.md ≤20 lines at session end.
